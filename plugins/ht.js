let handler = async (m, { conn, text }) => {
  ...
  if (m.chat in conn.hartatahta) throw 'Ainda há um trabalho em andamento\nTexto do Tesouro do Trono\neste chat ... espere terminar'
  else conn.hartatahta[m.chat] = true
  m.reply('_Está fazendo..._\n*Por favor, aguarde cerca de 1 minuto*\n O atraso está ok? @₩erbert')
  try {
    let img = await ht(text ? text : ':v')
    conn.sendFile(m.chat, img, 'NINJA.png', 'Crédito: *@ ₩erbert*\nFeito com FFmpeg', m)
  } finally {
    delete conn.hartatahta[m.chat]
  }
}
handler.help = ['plaquinhaninja <teks>']
handler.tags = ['tools']
